...
class DataGenerator(BaseDataGenerator):
    ...
    def load_NAB_dataset(self, dataset, y_scale=6):
        ...
        plt.savefig(f"{self.config['result_dir']}/raw_data_normalised.pdf")
        plt.close()
        ...
        self.train_set_vae = {
            'data': np.expand_dims(rolling_windows[idx_train], -1).astype(np.float32)
        }
        self.val_set_vae = {
            'data': np.expand_dims(rolling_windows[idx_val], -1).astype(np.float32)
        }
        self.test_set_vae = {
            'data': np.expand_dims(rolling_windows[idx_val[:self.config['batch_size']]], -1).astype(np.float32)
        }
        ...
        self.train_set_lstm = {
            'data': np.expand_dims(lstm_seq[idx_train], -1).astype(np.float32)
        }
        self.val_set_lstm = {
            'data': np.expand_dims(lstm_seq[idx_val], -1).astype(np.float32)
        }
    ...
    def plot_time_series(self, data, time, data_list):
        ...
        plt.savefig(f"{self.config['result_dir']}/raw_training_set_normalised.pdf")
        plt.close()