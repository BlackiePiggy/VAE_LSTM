...
class DataGenerator(BaseDataGenerator):
    def __init__(self, config):
        ...
        # Load data: generate train_set, val_set and test_set
    ...
    def load_NAB_dataset(self, dataset, y_scale=6):
        ...
        # Normalize the dataset by training set mean and std
        ...
        axs.plot(np.mean(data['idx_split']) * np.ones(20), np.linspace(-y_scale, y_scale, 20), 'b--')
        ...
        axs.set_xlabel(f"timestamp (every {data['t_unit']})")
        axs.set_ylabel("readings")
        axs.set_title(f"{dataset} dataset\n(normalised by train mean {train_m:.4f} and std {train_std:.4f})")
        ...
        self.train_set_vae = {'data': np.expand_dims(rolling_windows[idx_train], -1)}
        self.val_set_vae = {'data': np.expand_dims(rolling_windows[idx_val], -1)}
        self.test_set_vae = {'data': np.expand_dims(rolling_windows[idx_val[:self.config['batch_size']]], -1)}
        ...
        for k in range(self.config['l_win']):
            ...
            for i in range(n_train_lstm):
                ...
                for j in range(self.config['l_seq']):
                    cur_seq[j] = data['training'][
                                 k + self.config['l_win'] * (j + i): k + self.config['l_win'] * (j + i + 1)]
        ...
        self.train_set_lstm = {'data': np.expand_dims(lstm_seq[idx_train], -1)}
        self.val_set_lstm = {'data': np.expand_dims(lstm_seq[idx_val], -1)}
